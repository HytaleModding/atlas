 ...
 * backend caps at 100; once `limit` reaches 100, no more pagination. */
const PAGE_SIZE = 10;
const MAX_LIMIT = 100;
/** Cap on persisted query history. */
...
export const useSearchStore = create<SearchState>()(
  ...
  runNow: async (q, slot, limit) => {
    ...
    try {
      ...
      // requesting more won't pad with garbage; we'll just get the
  ...
  setSelected: (hit) => {
    ...
    truncated.push(hit);
    // Cap the stack to keep it bounded; oldest entries fall off.
